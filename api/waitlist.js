"use strict";
const waitlistSns = require("controllers/sns-controller");
...
module.exports.put = async event => {
  try {
    ...
    const updatedWaitlistEntry = await waitlistDBHelpers.updateWaitlistStatus(
      id,
      status
    );
    if (status.called && !status.seated) {
      await waitlistSns.notifyUserToBeSeated(
        updatedWaitlistEntry.Attributes.PhoneNumber,
        updatedWaitlistEntry.Attributes.FirstName
      );
    }
    return successResponse("Retrieved from database!", updatedWaitlistEntry);
  } catch (error) {
    return errorResponse(
      "Failed to get detailed information from the database!"
    );
  }
};
...
module.exports.getAll = async event => {
  try {
    const allWaitlistData = await waitlistDBHelpers.getAllFromWaitlist(
      "Tasty Garden"
    );
...
module.exports.add = async event => {
  ...
  if (
    ...
    return errorResponse(
      "Failed to add to the waitlist because of data input errors!"
    );
  }
  ...
  try {
    ...
    return errorResponse(
      "Failed to add to the waitlist because of database error!"
    );
  }
};
